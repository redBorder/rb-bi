...
public class KafkaSpout implements IRichSpout {
     ...
     * spout is opened.
     */
    public KafkaSpout(String zkConnect, String topic, String groupId) {
        _topic=topic;
        _groupId=groupId;
     ...
     */
    protected void createConsumer(String zkConnect, String groupId) {
        final Properties consumerConfig = createKafkaConfig(zkConnect, groupId);
    ...
    public void open(final Map config, final TopologyContext topology, final SpoutOutputCollector collector) {
        ...
        createConsumer(_zkConnect, _groupId);
    ...
    public void nextTuple() {
        ...
        if (!_queue.isEmpty() || (_inProgress.isEmpty() && fillBuffer())) {
            ...
            if (nextId != null) {
                ...
                try {
                    _collector.emit(new Values(new String((byte[]) message, "UTF-8")), nextId);
                } catch (UnsupportedEncodingException ex) {
                    java.util.logging.Logger.getLogger(KafkaSpout.class.getName()).log(Level.SEVERE, null, ex);
                }
