...
import net.redborder.storm.util.state.IStateSingleKeyBuilder;
...
public class MemcachedMultipleState<T> implements IBackingMap<T> {
    ...
    protected static class Factory implements StateFactory {
        ...
        public State makeState(Map conf, IMetricsContext context, int partitionIndex, int numPartitions) {
            MemcachedMultipleState s;
            try {
                s = new MemcachedMultipleState(makeMemcachedClient(_opts, _servers), _opts, _ser);
...
//            IBackingMap c = new CachedMap(s, _opts.localCacheSize);
    ...
    public MemcachedMultipleState(Client client, Options opts, Serializer<T> ser) {
        _client = client;
        _opts = opts;
        _ser = ser;
