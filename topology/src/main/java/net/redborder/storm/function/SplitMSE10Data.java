...
public class SplitMSE10Data extends BaseFunction {
    ...
    public void prepare(Map conf, TridentOperationContext context) {
        logger = RbLogger.getLogger(SplitMSE10Data.class.getName());
    }
    ...
    public void execute(TridentTuple tuple, TridentCollector collector) {
        ...
        for (Map<String, Object> notification : notifications){
            if(notification.get("notificationType").equals("association")){
                logger.severe("Mse10 event this event is a association, emitting: [" + notification.size() + ", " + "null]");
                collector.emit(new Values(notification, null));
            }else if(notification.get("notificationType").equals("locationupdate")){
                logger.severe("Mse10 event this event is a locationupdate, emitting: [null" + ", " + notification.size() + "]");
